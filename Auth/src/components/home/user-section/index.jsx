...
const ProfilePage = () => {
  ...
  // Function to generate QR code and measure time
  const generateQRCode = () => {
    const startTime = performance.now();
    const qrValue = `UserID: ${profileData.userId}\nBasketID: ${profileData.basketId}`;

    console.log(qrValue);
    // Simulate QR code generation (actual rendering happens in JSX)
    const qrComponent = (
      <QRCodeCanvas 
        value={qrValue} 
        size={230} 
    ...
    console.log(`QR code generation took: ${(endTime - startTime).toFixed(2)} ms`);
    return qrComponent;
  };

  // Use useEffect to log timing after render if needed
  useEffect(() => {
    if (isValidData) {
      const startTime = performance.now();
      // QR code is rendered in the JSX below, so we measure the effect of its inclusion
      const endTime = performance.now();
      console.log(`QR code preparation (useEffect) took: ${(endTime - startTime).toFixed(2)} ms`);
    }
  }, [isValidData]);
